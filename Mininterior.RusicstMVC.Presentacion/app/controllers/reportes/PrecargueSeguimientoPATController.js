app.controller('PrecargueSeguimientoPATController', ['$scope', 'APIService', 'UtilsService', 'i18nService', '$http', 'uiGridConstants', '$interval', 'uiGridGroupingConstants', '$log', '$uibModal', 'authService', 'Upload', 'ngSettings', function ($scope, APIService, UtilsService, i18nService, $http, uiGridConstants, $interval, uiGridGroupingConstants, $log, $uibModal, authService, Upload, ngSettings) {
        ...
        Upload.upload({
            ...
            switch (Resultado.data.estado) {
                case 0:
                    ...
                    break;
                case 1:
                    var mensaje = { msn: "El proceso se realizo satisfactoriamente. " + Resultado.data.respuesta, tipo: "alert alert-success" };
                    mostrarMensaje(mensaje);
                    break;
                case 2:
                    var mensaje = { msn: "El proceso se realizo satisfactoriamente. " + Resultado.data.respuesta, tipo: "alert alert-success" };
    ...
    $scope.guardarDatos = function () {
        if ($scope.file) {
            ...
            $scope.url = '/api/Reportes/PrecargueSeguimientoPAT/CargarArchivo';
        ...
        var modalInstance = $uibModal.open({
            ...
            resolve: {
                datos: function () {
                    ...
                    var titulo = 'Cargar precargue de preguntas seguimiento PAT';
                    var url = '/api/TableroPat/IngresarTablero';
                    var entity = modelo;
                    var msn = "¿Está seguro de cargar el archivo? Tenga en cuenta que si ya existe un precargue, se borrarán los registros y se reemplazarán con estos nuevos.";

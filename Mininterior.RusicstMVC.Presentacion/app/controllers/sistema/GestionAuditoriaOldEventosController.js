app.controller('GestionAuditoriaOldEventosController', ['$scope', 'APIService', 'UtilsService', 'i18nService', '$http', 'uiGridConstants', '$interval', 'uiGridGroupingConstants', '$log', '$uibModal', '$timeout', '$templateCache', 'authService', function ($scope, APIService, UtilsService, i18nService, $http, uiGridConstants, $interval, uiGridGroupingConstants, $log, $uibModal, $timeout, $templateCache, authService) {
    ...
    function getCategory() {
        var url = '/api/General/Listas/CategoryOld?audUserName=' + authService.authentication.userName + '&userNameAddIdent=' + authService.authentication.userNameAddIdent;
        var servCall = APIService.getSubs(url);
        servCall.then(function (datos) {
    ...
    var getDatos = function () {
        ...
        var url = '/api/Sistema/LogXCategoriaOld/';
    ...
    var getDatosExportar = function () {
        ...
        if ($scope.usuarioSeleccionado == null)
            var url = $scope.serviceBase + '/api/Sistema/LogXCategoriaExportarOld/?pCategoria=' + $scope.registro.CategoryID + '&pUserName=' + null + '&pFechaIni=' + FechaIni + '&pFechaFin=' + FechaFin;
        else
            var url = $scope.serviceBase + '/api/Sistema/LogXCategoriaExportarOld/?pCategoria=' + $scope.registro.CategoryID + '&pUserName=' + $scope.usuarioSeleccionado + '&pFechaIni=' + FechaIni + '&pFechaFin=' + FechaFin;
        window.open(url)
    }
    ...
    $scope.openVerDetalles = function (entity) {
        var modalInstance = $uibModal.open({
            ...
            controller: 'ModalVerAuditoriaOldController',
    ...
    $scope.today = function () {
        $scope.registro.FechaFin = new Date(2017, 11, 31);
        $scope.registro.FechaInicio = new Date(2017,11,01);
    };
    $scope.today();
...
app.controller('ModalVerAuditoriaOldController', ['$scope', 'APIService', '$filter', '$log', '$uibModalInstance', '$http', 'entity', function ($scope, APIService, $filter, $log, $uibModalInstance, $http, entity) {
    //--------------------Inicio-------------------------------------
    $scope.registro = entity;

    //// Carga de combo de categorías o acciones
    function getAuditoria() {
        var url = '/api/Sistema/LogOld?logId=' + entity.LogID;
        var servCall = APIService.getSubs(url);
        servCall.then(function (datos) {
            entity = datos;
            $scope.items = [

...
{
    public class FuncDISABLE : IFunc
    {
        ...
        public object Invoke(FuncContext fctx)
        {
            if (Arguments.Length != 1)
                throw new Exception("La función DISABLE requiere un solo parámetro");

            var qName = Arguments[0];
            if (qName.StartsWith("@") == false)
                throw new Exception("La función DISABLE requiere un solo parámetro de tipo referencia, que comience por @");

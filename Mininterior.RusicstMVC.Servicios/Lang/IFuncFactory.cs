    ...
    public class IFuncFactory
    {
        ...
        public static IFunc CreateFunc(string expression)
        {
            ...
            if (match.Success)
            {
                ...
                var funcObject = ReflectionUtils.Initialize(typeof(IFuncFactory).Assembly.FullName, "Mininterior.RusicstMVC.Servicios.Lang.Func" + funcName);
        ...
        public static string[] ReturnValue(string expression)
        {
            var deps = new List<string>();
            var b = new StringBuilder();
            var isReading = false;
